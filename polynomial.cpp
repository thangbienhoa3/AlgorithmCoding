...
struct Node* addPolynomial(struct Node* first, struct Node* second)
{
    ...
    Node *polyHead = NULL,*polyLast = polyHead;
    Node *it1 = first, *it2 = second;
    double tmpcoff = 0; 
    int tmpPow = 0;
    while(it1 != NULL && it2 != NULL){
        if(it1->soMu > it2->soMu){
            tmpcoff = it1->heSo;
            tmpPow = it1->soMu;
            it1 = it1->next;
        }
        else if(it1->soMu < it2->soMu){
            tmpcoff = it2->heSo;
            tmpPow = it2->soMu;
            it2 = it2->next;
        }
        else if(it2->soMu == it1->soMu){
            tmpcoff = it1->heSo + it2->heSo;
            tmpPow = it1->soMu;
            it1 = it1->next;
            it2 = it2->next;
        }
        ...
        Node *newPoly = createNewNode(tmpcoff,tmpPow);
        addElementLast(&polyHead,&polyLast,newPoly);
    }
    while(it1 != NULL){
        tmpcoff = it1->heSo;
        tmpPow = it1->soMu;
...
struct Node* multiplyPolynomialWithNode(struct Node* head, struct Node* multiplier)
{
    ...
    double tmpCoff = 0.0;
    while(it1 != NULL){
        tmpPow = it1->soMu + multiplier->soMu;
        tmpCoff = it1->heSo * multiplier->heSo;
        Node *newPoly = createNewNode(tmpCoff,tmpPow);
        addElementLast(&polyHead,&polyLast,newPoly);
		it1 = it1->next;
...
{ 
	...
	while(it1 != NULL && it1->next != NULL){
		it2 = it1;
		while(it2->next != NULL){
			if(it1->soMu == it2->next->soMu){
				it1->heSo = it1->heSo + it2->next->heSo;
				dup = it2->next;
...
void Sort(Node** start){
	...
	map <int,double> mp;
	while(it2 != NULL){
		if(mp.find(it2->soMu) == mp.end()){
			mp[it2->soMu] = it2->heSo;
		}
		else mp[it2->soMu] += it2->heSo;
		it2 = it2->next;
	}
...
struct Node* multiplyTwoPolynomial(struct Node* first, struct Node* second)
{
	// SINH VIÊN CODE TẠI ĐÂY
	Node *it1 = first, *it2 = second;
	Node *pHead = NULL, *pLast = pHead;
	while(it1 != NULL){
		while(it2 != NULL){
			int tmpPow;
			double tmpCoff;
			tmpCoff = it1->heSo * it2->heSo;
			tmpPow = it1->soMu + it2->soMu;
			Node *newPoly = createNewNode(tmpCoff,tmpPow);
        	addElementLast(&pHead,&pLast,newPoly);
			it2 = it2->next;
		}
		it2 = second;
		it1 = it1->next;
